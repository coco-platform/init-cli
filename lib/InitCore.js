...
class InitCore {
  ...
  download() {
    ...
    const seed = `coco-template/${this.options.template}`;
  ...
  async render() {
    ...
    const templateFiles = tools.explorePatternFiles(templates, rootProject);
    ...
    const unlinkFiles = Array.isArray(ignores)
      ? [...ignores, 'coco.yml']
      : ['coco.yml'];

    unlinkFiles.forEach((ignore) => {
